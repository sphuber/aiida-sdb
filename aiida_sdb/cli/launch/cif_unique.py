...
def cif_unique(group_candidate, group_reference, contains, max_size, partial_occupancies, add):
    ...
    for formula, entries in sorted(mapping.items(), key=lambda x: x[0]):
        ordered[formula] = [entry[2] for entry in sorted(entries, key=lambda e: (e[0], e[1]))]
        ...
        for compound, structures in ordered.items():
            ...
            for i in range(nstructures):
                for j in range(i + 1, nstructures):
                    adjacent_matrix[i, j] = structure_similarity(matcher, structures[i], structures[j])
            ...
            for prototype, duplicates in unique[compound].items():
                if node.uuid in [dup.uuid for dup in duplicates]:
                    ...
                    for structure in duplicates:
                        extra.setdefault(structure.get_extra('source')['database'], []).append(structure.uuid)
                    for structure in duplicates:
